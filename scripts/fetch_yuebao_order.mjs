...
async function run() {
  console.log('Fetching HTML for yuebao_order/index?addtabs=1...');
  const resHtml = await fetchUrl('https://spotline888.org/coinht.php/yuebao_order/index?addtabs=1');
  console.log('HTML status:', resHtml.status, 'size:', resHtml.body.length);
  const htmlStr = resHtml.body.toString('utf-8');
  fs.writeFileSync('scripts/yuebao_order_inner.html', resHtml.body);

  const jsMatch = htmlStr.match(/jsname":"([^"]+)"/);
  console.log('JS Name:', jsMatch ? jsMatch[1] : 'None');

  console.log('Fetching JSON data for yuebao_order table...');
  const resJson = await fetchUrl('https://spotline888.org/coinht.php/yuebao_order/index?sort=id&order=desc&offset=0&limit=50', {
  ...
  if (jsMatch) {
    ...
    fs.writeFileSync('scripts/backend_yuebao_order.js', resJs.body);
  }
